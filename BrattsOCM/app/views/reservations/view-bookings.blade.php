                ...
                <div class="row">
                    <div class="col s12 m8 l9">
                        <table id="data-table-row-grouping" class="display" cellspacing="0" width="130%" style="align-self: center">
                            ...
                            <tbody>
                            @if(isset($dataBookings))
                                @foreach ($dataBookings as $details)
                                    <tr>
                                        <td class="booking_id">{{ $details['booking_id'] }}</td>
                                        <td class="full_name">{{ $details['fullname']}}</td>
                                        <td class="city">{{ $details['branch_code']}}</td>
                                        <td class="room_code">{{ $details['room_code']}}</td>
                                        <td class="price">{{ $details['checkin_time']}}</td>
                                        <td class="address">{{ $details['checkout_time']}}</td>



                                        <td><p><a class="waves-effect waves-light btn light-pink  cancel" >Cancel</a></p>

                                        </td>

                                        <td><p><a class="waves-effect waves-light btn light-pink refund-book" >Refund</a></p>
     ...
     $(document).ready(function() {
         ...
         $('.cancel').on('click', function () {
             ...
             if (r == true) {
                 ...
                 var booking_id = $(this).parent().parent().parent().find('.booking_id').html();
                 var room_code = $(this).parent().parent().parent().find('.room_code').html();
                 var full_name = $(this).parent().parent().parent().find('.full_name').html();
                 var branch_code = $(this).parent().parent().parent().find('.city').html();

                 $.get('{{URL::route('cancel-booking')}}', {
                     accommodation_id: booking_id,
                     roomCode: room_code,
                     ...
                     if (!cancelled) {
                         ...
                         window.location = '{{URL::route('view-booked-own-hotel')}}';
                     }
                 });
             } else {
...
/*    $(document).ready(function() {
        $('.refund-book').on('click', function () {
                    alert('hgfd');
                    var booking_id = $(this).parent().parent().parent().find('.booking_id').html();

                    $.get('{{URL::route('refund-booking')}}', {
                        accommodation_id: booking_id
                    }, function () {


                        Materialize.toast('<span>Email has sent to the Hotel Admin!</span>', 2000)
                        //window.location = '
                        {{URL::route('view-booked-own-hotel')}}';
